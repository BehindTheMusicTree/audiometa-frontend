...
import { AudioMetadataDetailedSchema, type AudioMetadataDetailed } from "@/schemas/audio-metadata";
...
export function useGetFullMetadata() {
  ...
  const getMetadata = useCallback(async (file: File): Promise<AudioMetadataDetailed> => {
    setIsPending(true);
    setError(null);
    try {
