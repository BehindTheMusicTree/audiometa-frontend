...
- **Env / deploy**: Removed app-level `NEXT_PUBLIC_*` vars for developer credit and org URL; header/footer use **`@behindthemusictree/assets`**. Build requires `AUDIOMETA_PYTHON_GITHUB_REPO_URL`, `NEXT_PUBLIC_POSTHOG_PROJECT_TOKEN`, and `NEXT_PUBLIC_POSTHOG_HOST`, plus `NEXT_PUBLIC_BACKEND_BASE_URL`, `NEXT_PUBLIC_BACKEND_ROOT_SEGMENT`, and `NEXT_PUBLIC_DOCS_BUNDLE_URL` (GitHub `BACKEND_BASE_URL` / `BACKEND_ROOT_SEGMENT` sync to the `NEXT_PUBLIC_*` names on Vercel; see `docs/DEPLOYMENT.md` and `.env.example`).
